...
BS_all <- BSseq(pos = data$V2, chr = data$V1, 
		M = as.matrix(data$V4,ncol=1), Cov = as.matrix(data$V5,ncol=1), sampleNames = "V2")

#====Parameters below will be modified per run
pData(BS_all)$Rep <- "V2"
validObject(BS_all)
pData(BS_all)
BS_all <- BSmooth(BS_all, mc.cores = 1, ns = TESTNS, h = TESTH, maxGap=100000, parallelBy="chromosome")
print(mem_used())
