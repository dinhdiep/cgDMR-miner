...
print "index\ttotal_depth\taverage_methylation\tweighted_average_methylation\tC_statistic\tempirical_p.value\tabsolute_difference\n";
...
while($line = <STDIN>){
  ...
  print $value, "\t", $sum_depth, "\t", $p, "\t", $o_m/$o_d, "\t", $chsq_final, "\t", $est_p, "\t", $max_mf-$min_mf, "\n";
}
